---
sidebar_label: "Spring AOP & Cache"
layout: default
---
# **AOP**
### AOP(Cross-Cutting)
...
- **value**: 캐싱 공간의 대표 명칭
- **key**: Spring Expression Language(SpEl)으로 key생성을 지정 (지정하지 않으면 모든 파라미터를 조합한 해시코드 값을 키로 생성)
- **condition**: 조건부 캐싱. SpEL로 지정하고 표현식이 true면 메서드를 캐시
...
| method | root object | 호출되는 메서 | `#root.method.name` |
...
| argument name | evaluation context | 메서드 인자의 이름. 어떤 이유로든 이름을 사용할 수 없다면(예: 디버깅 정보가 없는 경우) `a<#arg>`에서 인자 이름을 사용할 수도 있고 `#arg`은 (0부터 시작하는) 인자의 인덱스를 의미한다. | iban나 a0 (p0를 사용하거나 별칭으로 `p<#arg>` 형식을 사용할 수 있다) |
