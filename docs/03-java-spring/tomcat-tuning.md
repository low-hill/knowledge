...
```
### TCP parameters
* 하위 값을 /etc/sysctl.conf 또는 `/etc/sysctl.d/<config_name>.conf`에 추가하고 `sysctl -p`를 실행하여 적용한다.
```
...
### TCP port range
* 하위 값을 /etc/sysctl.conf or `/etc/sysctl.d/<config>.conf`에 추가하고 `sysctl -p`로 적용한다.
