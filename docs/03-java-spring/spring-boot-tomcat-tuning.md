---
sidebar_label: "Spring Boot에서 Tomcat 튜닝"
layout: default
---
Spring Boot 환경에서는 내장 Tomcat의 구성과 설정을 적절히 튜닝하여 애플리케이션의 처리 성능과 리소스 활용도를 개선할 수 있다. 주요 튜닝 포인트는 다음과 같다.

- 스레드 풀 설정: maxThreads, minSpareThreads, acceptCount 등을 조정하여 동시 요청 처리량과 대기 큐를 최적화한다.
- 커넥터 설정(NIO/APR): 기본 NIO 커넥터를 사용하되, 높은 네트워크 처리 성능이 필요한 경우 APR/native를 고려하여 I/O 효율을 개선한다.
- JVM 최적화: 힙 메모리(-Xms, -Xmx)와 GC 옵션(G1GC 등)을 조정하여 GC로 인한 stop-the-world 지연을 최소화하여 응답 시간의 일관성을 확보한다.
